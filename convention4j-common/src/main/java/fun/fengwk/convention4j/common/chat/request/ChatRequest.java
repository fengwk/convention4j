...
public class ChatRequest {
    ...
    public ChatRequest copy() {
        ...
        chatRequest.setMessages(NullSafe.map(getMessages(), ArrayList::new));
        chatRequest.setModel(getModel());
        chatRequest.setFrequency_penalty(getFrequency_penalty());
        chatRequest.setLogit_bias(NullSafe.map(getLogit_bias(), HashMap::new));
        ...
        chatRequest.setResponse_format(NullSafe.map(getResponse_format(), HashMap::new));
        chatRequest.setSeed(getSeed());
        chatRequest.setStop(NullSafe.map(getStop(), ArrayList::new));
        chatRequest.setStream(getStream());
        chatRequest.setTemperature(getTemperature());
        chatRequest.setTop_p(getTop_p());
        chatRequest.setTools(NullSafe.map(getTools(), ArrayList::new));
        chatRequest.setTool_choice(NullSafe.map(getTool_choice(), HashMap::new));
        chatRequest.setUser(getUser());
        chatRequest.setFunction_call(NullSafe.map(getFunction_call(), HashMap::new));
        chatRequest.setFunctions(NullSafe.map(getFunctions(), ArrayList::new));
        return chatRequest;
    }
